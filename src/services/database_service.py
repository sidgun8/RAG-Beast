...
class DatabaseService:
        ...
        async with self._pool.acquire() as conn:
            async with conn.transaction():
                try:
                    ...
                    query = '''
                        INSERT INTO documents (id, title, content, content_type, file_path, metadata, embedding)
    ...
    async def get_all_documents(self, limit: int = 50, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        async with self._pool.acquire() as conn:
            query = '''
                SELECT id, title, content_type, file_path, metadata, created_at, updated_at
                FROM documents 
        ...
        async with self._pool.acquire() as conn:
            async with conn.transaction():
                try:
                    # Delete chunks first
                    await conn.execute(
                        "DELETE FROM documents WHERE metadata->>'parentId' = $1", 
                        document_id
                    )
    ...
    async def semantic_search(self, query_embedding: List[float], limit: int = 10, 
        ...
        async with self._pool.acquire() as conn:
            ...
            results = await conn.fetch(query, embedding_str, similarity_threshold, limit)
    ...
    async def search_by_metadata(self, metadata_query: Dict[str, Any], limit: int = 10) -> List[Dict[str, Any]]:
        ...
        async with self._pool.acquire() as conn:
            ...
            results = await conn.fetch(query, json.dumps(metadata_query), limit)
            return [dict(row) for row in results]
    ...
    async def get_similar_documents(self, document_id: str, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        async with self._pool.acquire() as conn:
            ...
            doc_query = 'SELECT embedding FROM documents WHERE id = $1'
            ...
            results = await conn.fetch(similar_query, embedding, document_id, limit)
            return [dict(row) for row in results]
